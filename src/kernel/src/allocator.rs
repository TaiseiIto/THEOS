...
impl<'a> ChunkList<'a> {
    ...
    fn dealloc(&'a mut self, address: usize) {
        let (deallocated_chunk, previous_chunk, next_chunk): (&mut Chunk, Option<&mut Option<Chunk>>, Option<&mut Option<Chunk>>) = self.get_deallocated_chunk(address);
    ...
    fn get_deallocated_chunk(&'a mut self, address: usize) -> (&mut Chunk, Option<&mut Option<Chunk>>, Option<&mut Option<Chunk>>) {
