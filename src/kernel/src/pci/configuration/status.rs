...
impl From<u16> for Register {
    fn from(status: u16) -> Self {
        let immediate_readiness: bool = status & Self::IMMEDIATE_READINESS_MASK != 0;
        let interrupt_status: bool = status & Self::INTERRUPT_STATUS_MASK != 0;
        let capabilities_list: bool = status & Self::CAPABILITIES_LIST_MASK != 0;
        let flag_66mhz_capable: bool = status & Self::FLAG_66MHZ_CAPABLE_MASK != 0;
        let fast_back_to_back_transactions_capable: bool = status & Self::FAST_BACK_TO_BACK_TRANSACTIONS_CAPABLE_MASK != 0;
        let master_data_parity_error: bool = status & Self::MASTER_DATA_PARITY_ERROR_MASK != 0;
        let devsel_timing: [bool; 2] = Self::DEVSEL_TIMING_MASK
            .iter()
            .map(|mask| status & mask != 0)
            .collect::<Vec<bool>>()
            .try_into()
            .expect("Can't get a PCI configuration status!");
        let signaled_target_abort: bool = status & Self::SIGNALED_TARGET_ABORT_MASK != 0;
        let received_target_abort: bool = status & Self::RECEIVED_TARGET_ABORT_MASK != 0;
        let received_master_abort: bool = status & Self::RECEIVED_MASTER_ABORT_MASK != 0;
        let signaled_system_error: bool = status & Self::SIGNALED_SYSTEM_ERROR_MASK != 0;
        let detected_parity_error: bool = status & Self::DETECTED_PARITY_ERROR_MASK != 0;
        Self {
            immediate_readiness,
            interrupt_status,
            capabilities_list,
