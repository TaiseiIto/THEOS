...
impl<'a> PageMapLevel4Entry<'a> {
    ...
    fn clone(&self, page_map_level_4_entry: &'a mut u64) -> Self {
        ...
        *page_map_level_4_entry = 
            ...
            | accessed_in_entry
            | restart_in_entry
            | page_directory_pointer_table_in_entry
            | execute_disable_in_entry;
        Self {
            present,
            virtual_address,
    ...
    fn divide_child(&mut self, virtual_address: usize) {
        assert!(virtual_address & (usize::MAX << Self::INDEX_SHIFT_BEGIN) == self.virtual_address);
        let writable: bool = true;
        let user_mode_access: bool = false;
        let page_write_through: bool = false;
        let page_cache_disable: bool = false;
...
impl<'a> PageDirectoryPointerEntry<'a> {
    ...
    fn clone(&self, page_directory_pointer_entry: &'a mut u64) -> Self {
        let Self {
            present,
            virtual_address,
            page_directory_pointer_entry: _,
            writable,
            user_mode_access,
            page_write_through,
            page_cache_disable,
        ...
        let page_directory_table_in_entry: u64 = page_directory_table_page
            .as_ref()
            .map_or(0, |page_directory_table_page| page_directory_table_page.physical_address());
        let execute_disable_in_entry: u64 = if execute_disable {
            Self::EXECUTE_DISABLE_MASK
        } else {
            0
